// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "./types/input";
import * as outputs from "./types/output";
import * as utilities from "./utilities";

export class LinkBranding extends pulumi.CustomResource {
    /**
     * Get an existing LinkBranding resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: LinkBrandingState, opts?: pulumi.CustomResourceOptions): LinkBranding {
        return new LinkBranding(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'sendgrid:index/linkBranding:LinkBranding';

    /**
     * Returns true if the given object is an instance of LinkBranding.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is LinkBranding {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === LinkBranding.__pulumiType;
    }

    /**
     * The DNS records used to authenticate the sending domain.
     */
    declare public /*out*/ readonly dns: pulumi.Output<outputs.LinkBrandingDn[]>;
    /**
     * Domain being authenticated.
     */
    declare public readonly domain: pulumi.Output<string>;
    /**
     * Indicates if this is the default link branding.
     */
    declare public readonly isDefault: pulumi.Output<boolean | undefined>;
    /**
     * The subdomain to use for this link branding.
     */
    declare public readonly subdomain: pulumi.Output<string>;
    /**
     * The username associated with this domain.
     */
    declare public /*out*/ readonly username: pulumi.Output<string>;
    /**
     * Indicates if this is a valid link branding or not. Set to `true` to attempt validation on first update.
     */
    declare public readonly valid: pulumi.Output<boolean>;

    /**
     * Create a LinkBranding resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: LinkBrandingArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: LinkBrandingArgs | LinkBrandingState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as LinkBrandingState | undefined;
            resourceInputs["dns"] = state?.dns;
            resourceInputs["domain"] = state?.domain;
            resourceInputs["isDefault"] = state?.isDefault;
            resourceInputs["subdomain"] = state?.subdomain;
            resourceInputs["username"] = state?.username;
            resourceInputs["valid"] = state?.valid;
        } else {
            const args = argsOrState as LinkBrandingArgs | undefined;
            if (args?.domain === undefined && !opts.urn) {
                throw new Error("Missing required property 'domain'");
            }
            resourceInputs["domain"] = args?.domain;
            resourceInputs["isDefault"] = args?.isDefault;
            resourceInputs["subdomain"] = args?.subdomain;
            resourceInputs["valid"] = args?.valid;
            resourceInputs["dns"] = undefined /*out*/;
            resourceInputs["username"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(LinkBranding.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering LinkBranding resources.
 */
export interface LinkBrandingState {
    /**
     * The DNS records used to authenticate the sending domain.
     */
    dns?: pulumi.Input<pulumi.Input<inputs.LinkBrandingDn>[] | undefined>;
    /**
     * Domain being authenticated.
     */
    domain?: pulumi.Input<string | undefined>;
    /**
     * Indicates if this is the default link branding.
     */
    isDefault?: pulumi.Input<boolean | undefined>;
    /**
     * The subdomain to use for this link branding.
     */
    subdomain?: pulumi.Input<string | undefined>;
    /**
     * The username associated with this domain.
     */
    username?: pulumi.Input<string | undefined>;
    /**
     * Indicates if this is a valid link branding or not. Set to `true` to attempt validation on first update.
     */
    valid?: pulumi.Input<boolean | undefined>;
}

/**
 * The set of arguments for constructing a LinkBranding resource.
 */
export interface LinkBrandingArgs {
    /**
     * Domain being authenticated.
     */
    domain: pulumi.Input<string>;
    /**
     * Indicates if this is the default link branding.
     */
    isDefault?: pulumi.Input<boolean | undefined>;
    /**
     * The subdomain to use for this link branding.
     */
    subdomain?: pulumi.Input<string | undefined>;
    /**
     * Indicates if this is a valid link branding or not. Set to `true` to attempt validation on first update.
     */
    valid?: pulumi.Input<boolean | undefined>;
}
